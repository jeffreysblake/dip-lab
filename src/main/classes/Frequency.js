...
class Frequency {
     ...
     */
    static applyFFT(imageData, width, height) {
     ...
     */
    static applyLowPassFilter(imageData, width, height, cutoff) {
     ...
     */
    static applyHighPassFilter(imageData, width, height, cutoff) {
        const fftResult = this.applyFFT(imageData, width, height);
     ...
     */
    static applyBandPassFilter(imageData, width, height, lowCutoff, highCutoff) {
        const fftResult = this.applyFFT(imageData, width, height);
     ...
     */
    static applyFrequencyFilter(imageData, width, height, filterType) {
        switch (filterType) {
            case 'lowpass':
     ...
     */
    static inverseFFT(fftData, width, height) {
     ...
     */
    static processFrequencyDomain(imageData, width, height) {
     ...
     */
    static applyCustomFrequencyFilter(imageData, width, height) {
     ...
     */
    static getFrequencySpectrum(imageData, width, height) {
        const fftResult = this.applyFFT(imageData, width, height);
     ...
     */
    static applyFrequencyConvolution(imageData, width, height) {
        const result = new Uint8ClampedArray(width * height * 4);
        for (let i = 0; i < imageData.length; i += 4) {
     ...
     */
    static applyAdvancedFrequencyFilter(imageData, width, height) {
        const result = new Uint8ClampedArray(width * height * 4);
        for (let i = 0; i < imageData.length; i += 4) {
     ...
     * @returns A Float32Array containing real and imaginary components for each frequency bin
     */
    static getFrequencyDomainRepresentation(imageData, width, height) {
        const fftResult = this.applyFFT(imageData, width, height);
